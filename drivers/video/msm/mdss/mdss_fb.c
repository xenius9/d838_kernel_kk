...
#if defined(CONFIG_B1_LGD_PANEL)
		/* workaround for reducing the board download time */
		/* factory boot && laf mode && lcd discontected */
		if(is_fboot && is_lafmode && !is_lcd_connected){
			pr_err(" factory boot && LAF mode && lcd disconnected status \n");
			return -EPERM;
		}
#endif
